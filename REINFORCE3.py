...
def main():
    ...
    for episode in range(max_episode_num):
        ...
        for steps in range(max_steps):
            ...
            if done:
                logprob = torch.log(policy_net(torch.tensor(states))[
                    list(range(len(states))), actions]).detach().sum()
                update_policy(policy_net, rewards, log_probs)
                logprob2 = torch.log(policy_net(torch.tensor(states))[
                    list(range(len(states))), actions]).detach().sum()
                # print(logprob2-logprob)
